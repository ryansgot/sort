...
/*
 * Changes the presort when any presort button is clicked.
 */
void checkPresortChanged() {
  if (nearlySortedBox.clickedOn(mouseX, mouseY, xOffset, yOffset)) {
    deselectPresortButtons();
    nearlySortedBox.select();
    startOrdering = PipeSet.NEARLY_SORTED;
  }  // end if
  else if (reverseSortedBox.clickedOn(mouseX, mouseY, xOffset, yOffset)) {
    deselectPresortButtons();
    reverseSortedBox.select();
    startOrdering = PipeSet.REVERSE_SORTED;
  }  // end else if
  else if (randomSortedBox.clickedOn(mouseX, mouseY, xOffset, yOffset)) {
    deselectPresortButtons();
    randomSortedBox.select();
    startOrdering = PipeSet.RANDOM_SORTED;
  }  // end else if
}  // end method checkPresortChanged

/*
 * Turns all of the presort box button colors to their default color
 * This is helper for checkPresortChanged.
 */
void deselectPresortButtons() {
  nearlySortedBox.deselect();
  reverseSortedBox.deselect();
  randomSortedBox.deselect();
}  // end method deselectRightAlgorithmButtons

/*
 * toggles the sound variable if the sound box is clicked.
 */
void checkSoundToggled() {
  if (soundToggleBox.clickedOn(mouseX, mouseY, xOffset, yOffset)) {
    if (soundToggleBox.boxSelected()) {
      soundToggleBox.deselect();
    }  // end if
    else {
      soundToggleBox.select();
    }  // end else
  }  // end if
}  // end method checkSoundToggled
